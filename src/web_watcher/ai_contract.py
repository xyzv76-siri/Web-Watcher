...
# ---------------------------------------------------------------------------
# MockProvider — deterministic stdlib-only test double
# ---------------------------------------------------------------------------
...
class MockProvider:
    ...
    Supported scenarios (case-sensitive):
        * ``"valid"``            — returns a well-formed judgment JSON
        * ``"invalid_json"``     — returns a syntactically broken string
        * ``"invalid_schema"``   — returns valid JSON with wrong types
        * ``"unsupported_value"``— returns valid JSON with out-of-domain value
        * ``"provider_error"``   — raises ``ProviderError``
    ...
    def invoke(
        ...
        if scenario == "invalid_schema":
            return ProviderResponse(
                content=json.dumps({"relevance": "wrong_type"})
            )
